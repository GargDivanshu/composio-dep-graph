...
4. fetch raw tools for the 2 required toolkits (fastest way to run is https://bun.sh/):
   - `pnpm fetch:tools` (writes `googlesuper_tools.json` + `github_tools.json`)
5. build the dependency graph + a visualization:
   - `pnpm build:graph` (writes `tool_graph.json`, `tool_graph.dot`, `tool_graph.html`)
   - profiling: `pnpm build:graph -- --profile`
6. quick sanity check:
   - `pnpm graph:stats` (fails if `edges=0`)
7. open the visualization (no server):
   - `pnpm view:graph`
...
See `ROUTER_POLICY.md`.

## performance notes

See `PERFORMANCE.md` for why naive edge inference becomes `O(N^2)` and how this repo avoids it with indexing.
